...
object MadeOfSufferingEnchantmentEventHandler {
    ...
    fun onBurningPhobiaEnchantmentEvent(event: LivingDamageEvent) {
        ...
        if (event.entity is Player && event.shouldReflectDamage()) {
            if (event.entity.isInLava || event.entity.remainingFireTicks > 0 && event.source == DamageSource.ON_FIRE) {
    ...
    fun onConstrainedHeartEnchantmentEvent(event: LivingDamageEvent) {
        ...
        if (event.entity is Player) {
            ...
            if (enchantmentLevel == 0) return
            if (event.source == DamageSource.ON_FIRE || event.source == DamageSource.IN_WALL || event.source == DamageSource.CRAMMING || event.source == DamageSource.FALL || event.source == DamageSource.FLY_INTO_WALL || event.source == DamageSource.WITHER || event.source == DamageSource.MAGIC || player.hasEffect(
                        MobEffects.POISON
                    )) {
    ...
    fun onPiercingFeverEnchantmentEvent(event: LivingDamageEvent) {
        ...
        if (event.entity is Player) {
            ...
            if (enchantmentLevel == 0) return
            if (event.source.isProjectile || event.source == DamageSource.CACTUS || event.source == DamageSource.SWEET_BERRY_BUSH) grandAbnormalJoyPoint(event, enchantmentLevel)
        }
    }
    ...
    fun onDestructionWorshipEnchantmentEvent(event: LivingDamageEvent) {
        ...
        if (event.entity is Player) {
            ...
            if (enchantmentLevel == 0) return
            if (event.source.isMagic || event.source.isExplosion || event.source == DamageSource.LIGHTNING_BOLT) {
                grandAbnormalJoyPoint(event, enchantmentLevel)
            }
        }
    }
