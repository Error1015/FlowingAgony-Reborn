...
object RootedInHatredEnchantmentEventHandler {
    ...
    fun doResentfulSoulEnchantmentEvent(event: LivingDamageEvent) {
        ...
        if (event.entity is Player && event.source != DamageSource.OUT_OF_WORLD && event.source.msgId == "burial_object_curse") {
    ...
    fun doTooResentfulToDieEnchantmentEvent(event: LivingDamageEvent) {
        ...
        if (event.entity is Player && event.entity != event.source.entity && event.source != DamageSource.OUT_OF_WORLD && event.source.msgId != "burial_object_curse") {
            val player = event.entity as Player
