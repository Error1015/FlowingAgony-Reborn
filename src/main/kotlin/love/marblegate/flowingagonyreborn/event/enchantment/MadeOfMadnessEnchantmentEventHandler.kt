...
object MadeOfMadnessEnchantmentEventHandler {
    ...
    fun onCuttingWatermelonDreamEnchantmentEventDropHeadAndExtraLoot(event: LivingDeathEvent) {
        ...
        if (event.source.msgId == "cutting_watermelon_dream" && event.source.entity is Player && event.entity.supportHeadDrop()) {
            val player = event.source.entity as Player
            ...
            if (player.isItemEnchanted(CuttingWatermelonDreamEnchantment, EquipmentSlot.MAINHAND)) {
                ...
                if (fortuneLevel > 0) dropLoot(event.entity, player, DamageSource.playerAttack(player), fortuneLevel)
            }
        }
    }
    ...
    private fun dropLoot(entity: LivingEntity, player: Player, source: DamageSource, lootLevel: Int) {
        ...
        val lootTable = entity.level.server?.lootTables?.get(resourceLocation) ?: return
        val builder = getLootContextBuilder(entity, player, source)
        val context = builder.create(LootContextParamSets.ENTITY)
    ...
    private fun getLootContextBuilder(entity: LivingEntity, player: Player, source: DamageSource): LootContext.Builder {
        var builder = (LootContext.Builder(entity.level as ServerLevel)).withRandom(entity.getRandom()).withParameter(LootContextParams.THIS_ENTITY, entity)
                .withParameter(LootContextParams.ORIGIN, entity.position()).withParameter(LootContextParams.DAMAGE_SOURCE, source).withOptionalParameter(LootContextParams.KILLER_ENTITY, source.entity)
                .withOptionalParameter(LootContextParams.DIRECT_KILLER_ENTITY, source.directEntity)
        builder = builder.withParameter(LootContextParams.LAST_DAMAGE_PLAYER, player).withLuck(player.luck)
        return builder
    }
